<script lang="ts">
    ...
    // Props
    export let initialMemoryVerse: string | null | undefined = undefined;
    export let initialReadingLogEntries: { read_date: string; reading_book?: string | null; reading_chapter?: string | null }[] | undefined = undefined;
    export let initialError: string | null | undefined = undefined;
  
    export let bibleActionData: {
    ...
    function processReadingLogForGraph(
      ...
      console.log("BibleStudyWidget: processReadingLogForGraph called with entries (first 5):", JSON.stringify(logEntries.slice(0,5)));
      ...
      // Go back (WEEKS_TO_DISPLAY_BIBLE_LOG - 1) * 7 days to get to the start of the period, then align to Sunday.
      ...
      for (const entry of logEntries) {
        ...
        // Only map entries that could potentially fall within our graph window
        // (a bit wider than graphStartDate to catch all relevant entries for the map)
        const checkStartDate = new Date(graphStartDate);
        checkStartDate.setDate(graphStartDate.getDate() - 7); // check a bit before for safety with map
      ...
      let calculatedReadDaysInPeriod = 0; // Use a local variable for summing
      ...
      for (let i = 0; i < WEEKS_TO_DISPLAY_BIBLE_LOG; i++) {
        ...
        for (let j = 0; j < 7; j++) { // 7 days a week
          ...
          if (logData) {
              ...
              dayCount = 1;
              calculatedReadDaysInPeriod++; // Increment here
          }
          
          // Only add day if it's within the overall display window (up to today)
          week.contributionDays.push({
            date: dateString,
            count: dayCount,
            color: (currentDayIterator > today) ? 'transparent' : dayColor, // Future days transparent
            book: logData?.book,
      ...
      totalReadDaysInPeriod = calculatedReadDaysInPeriod; // Assign the final sum
      console.log("BibleStudyWidget: Graph processed. Displaying", readingGraphWeeks.length, "weeks. Total read in period:", totalReadDaysInPeriod);
    }
  
    // --- Reactive Updates ---
    // Initialize memoryVerseInput when the prop is first available or changes
    ...
    $: {
      if (bibleActionData?.bibleSettingsSuccess) {
        // activeTab = 'log'; // Optionally switch tab
      }
      if (bibleActionData?.logReadingSuccess) {
        logBookInput = '';
        logChapterInput = '';
        console.log("BibleStudyWidget: logReadingSuccess detected from bibleActionData.");
        // The graph update is now handled by the reactive statement below watching initialReadingLogEntries
      }
    }
  
    // This is the primary reactive statement for updating the graph.
    // It runs whenever the `initialReadingLogEntries` prop changes its reference.
    $: {
      console.log("BibleStudyWidget: Reactive block for initialReadingLogEntries triggered. Entries (first 5):", JSON.stringify(initialReadingLogEntries?.slice(0,5)));
    ...
    onMount(() => {
      memoryVerseInput = initialMemoryVerse || '';
      // Initial graph processing is handled by the reactive statement above when initialReadingLogEntries is first set.
      console.log("BibleStudyWidget: Mounted. Initial log entries (first 5 on mount):", JSON.stringify(initialReadingLogEntries?.slice(0,5)));
    });
  ...
  <div class="flex flex-col h-full">
    ...
    {#if initialError && activeTab !== 'settings'}
      <p class="text-red-400 text-sm text-center p-2 mb-4">{initialError}</p>
    {/if}
      ...
      {#if activeTab === 'log'}
        <div class="p-1 bg-gray-700/30 rounded-lg">
          ...
          <form method="POST" action="?/logBibleReading" class="space-y-3 px-3 pb-3">
            ...
            {#if bibleActionData?.logReadingError}<p class="text-red-400 text-xs pt-1">{bibleActionData.logReadingError}</p>{/if}
            {#if bibleActionData?.logReadingSuccess}<p class="text-green-400 text-xs pt-1">Reading logged for today!</p>{/if}
          </form>
        </div>
      ...
      {:else if activeTab === 'settings'}
        <div class="p-1 bg-gray-700/30 rounded-lg">
          ...
          <form method="POST" action="?/saveBibleSettings" class="space-y-3 px-3 pb-3">
            ...
            {#if bibleActionData?.bibleSettingsError}<p class="text-red-400 text-xs pt-1">{bibleActionData.bibleSettingsError}</p>{/if}
            {#if bibleActionData?.bibleSettingsSuccess && activeTab === 'settings'}<p class="text-green-400 text-xs pt-1">Settings saved!</p>{/if}
          </form>
        </div>
      {/if}
    </div>
